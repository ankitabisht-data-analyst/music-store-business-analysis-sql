...
--  Each block contains: Business Question → QUERY SOLUTION
...
SELECT
    ...
    DENSE_RANK() OVER (
        ORDER BY AVG(total) DESC)  AS aov_rank
FROM invoice
...
WITH date_bounds AS (SELECT
        ...
        DATE_ADD( MIN(invoice_date),
            INTERVAL DATEDIFF(MAX(invoice_date), MIN(invoice_date)) / 2 DAY )  AS mid_date
    FROM invoice),
genre_half_sales AS (
    SELECT
        g.name    AS genre,
...
SELECT
    ...
    ROUND( SUM(CASE WHEN g.name = 'Rock' THEN 1 ELSE 0 END) / COUNT(pt.track_id) * 100 , 1)  AS rock_pct
...
SELECT
    ...
    ROUND( SUM(il.unit_price * il.quantity) / SUM(SUM(il.unit_price * il.quantity)) OVER () * 100, 1)      AS revenue_share_pct
FROM media_type   mt
