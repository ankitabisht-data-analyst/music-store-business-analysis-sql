...
--  Analyst: 50-year veteran approach — indexes first, then queries
-- ============================================================


-- ============================================================
-- SECTION 0 — RECOMMENDED INDEXES
-- Run these once after loading data. They dramatically speed up
-- all JOIN-heavy analytical queries below.
-- ============================================================
...
--           take the first row. No aggregation needed.
-- ─────────────────────────────────────────────────────────────
SELECT
    employee_id,
    first_name,
...
-- Return all countries ranked by invoice count (ties included).
-- ─────────────────────────────────────────────────────────────
SELECT
...
-- Strategy: DISTINCT on (email, first_name, last_name) avoids
--   duplicates when a customer bought Rock multiple times.
-- ─────────────────────────────────────────────────────────────
SELECT DISTINCT
    c.email,
    c.first_name,
...
-- Strategy: subquery computes AVG once; optimizer typically
--           runs it as a constant.
-- ─────────────────────────────────────────────────────────────
SELECT
    name,
    milliseconds
...
-- Strategy: CTEs keep the logic readable without nested
--           subqueries. The final join is a straight
--           grouped aggregate.
-- ─────────────────────────────────────────────────────────────
...
-- Strategy: 
--   Step 1 — count purchases per (country, genre).
--   Step 2 — rank within each country using RANK() so ties are
--             preserved (RANK = 1 means top genre or tied top).
-- ─────────────────────────────────────────────────────────────
...
-- Strategy: window function RANK() partitioned by country so
--           the optimizer avoids a self-join.
-- ─────────────────────────────────────────────────────────────
